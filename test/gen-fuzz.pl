...
# Each case is a directory containing "argv" (NUL-separated arguments,
# possibly empty) and "stdin" (raw bytes fed to standard input).
use strict;
use warnings;
...
for (1 .. 150) {
    my @args;
    # Widths below 2 and a first message word of "0" are deliberate fixes
    # (see README) and diverge from the reference; the fixed cases in
    # run.sh cover them, so the fuzz stays inside the parity contract.
