/*
 * cowsay.wasm: a C reimplementation of cowsay 3.03 (c) 1999-2000 Tony Monroe,
 * written for compilation to wasm32-wasip1.
 *
 * Output contract: byte-identical to cowsay 3.03 running under a modern Perl
 * (Text::Wrap "modern" algorithm, 2018.6 or later) for ASCII input, except
 * for the deliberate fixes listed in README.md section "Deliberate fixes".
 * For non-ASCII input, widths are counted in Unicode codepoints and UTF-8
 * sequences are never split; see README.md section "UTF-8 behavior".
 *
 * The remaining quirks of the original are part of the contract and are
 * reproduced deliberately; each carries a comment naming the Perl behavior
 * it mirrors.  Every departure carries a "Deliberate fix" comment.
 *
...
/* ---------- UTF-8 units ----------
 *
 * A unit is one codepoint; a byte that does not begin a valid sequence
 * (a stray continuation byte, a truncated sequence) is one unit by itself.
 * Overlong encodings are not rejected: the goal is only to never split
 * a sequence, not to validate it.
 */
...
/* First `units` codepoints of s, as a fresh string (Perl substr($s, 0, 2),
 * lifted from bytes to codepoints). */
...
/* ---------- Text::Wrap port ----------
 *
 * A restricted port of Text::Wrap 2024.001 wrap()/fill() with ip = xp = "",
 * $break = whitespace, $huge = 'wrap', $separator = "\n".
 * The input domain is what cowsay feeds it: no tabs (fill collapses them,
 * and the -l listing never contains any), so the expand/unexpand round trip
 * of the original is a no-op and is omitted.
...
static char *wrap_text(const char *t, size_t n, long *columns) {
    /* Deliberate fix: below two columns the reference interpolates a negative
     * regex quantifier into Text::Wrap's matcher and ends up returning its
     * argument count, turning the message into "3".  Any smaller width
     * behaves as 2, silently, matching Text::Wrap's own stated minimum. */
    ...
    for (;;) {
        ...
        if (p == n) break;
        /* Greedy match: the longest prefix of at most ll units whose end is
         * followed by a break or the end of the string. */
        size_t q = pos;
        long units = 0;
        ...
        if (best != (size_t)-1) {
            ...
            /* With ll >= 1 the walk always ends at a break candidate or at
             * ll units, so this is unreachable. */
            fputs("cowsay: internal wrap error\n", stderr);
            exit(70);
        }
        first = 0;
    }
    /* $r .= $remainder: a trailing single break survives (wrap("a ") is "a "),
     * while breaks skipped by the loop condition are dropped (wrap(" ") is ""). */
...
static char *fill_lines(char **lines, size_t nlines, long *columns) {
    ...
    /* split(/\n\s+/, $t): a newline followed by whitespace separates
     * paragraphs; Perl split keeps a leading empty field and drops trailing
     * empty fields. */
    List paras = {0};
...
/* Text::Tabs expand() with tabstop 8: between tab-separated segments, pad to
 * the next multiple of 8.  The original tracks only the previous segment's
 * length; the padding realigns every boundary to a tabstop, so that length
 * modulo 8 equals the column modulo 8. */
...
/* Consumes options from argv[1..]; returns the index of the first remaining
 * argument.  Mirrors Getopt::Std: clustered flags, "--" terminator (also when
 * produced by rewriting, as in "-b-"), "Unknown option: X" warnings with
 * parsing continuing, and a missing option argument becoming the empty
 * string without a message. */
static int getopts(int argc, char **argv, Opts *o) {
    ...
    for (;;) {
        ...
        if (hit) {
            ...
            /* Getopt::Std also handles --help/--version here; their output
             * embeds the host Perl version, so they are out of the
             * byte-parity contract (see README) and fall through as unknown. */
...
static void display_usage(void) {
    /* Deliberate fix: the reference exits 255 (Perl die with $! == 0), which
     * WASI preview 1 cannot represent, and its usage text carries a stray
     * trailing space from the heredoc.  This exits with EX_USAGE instead. */
    fprintf(stderr,
...
static char *get_cow(const char *f) {
    ...
    if (strchr(f, '/')) {
        /* Deliberate fix: the reference runs `do $full` without checking
         * existence, so a missing path silently prints no cow and exits 0;
...
static void print_name_list(List *names) {
    ...
    /* list_cowfiles runs before -W is applied, so this wraps at the
     * Text::Wrap default of 76 columns regardless of -W. */
    long cols = 76;
...
static char *construct_balloon(char **lines, size_t n, int think) {
    ...
    /* max stays -1 for an empty message: sprintf("%--1s", "") pads to one
     * column, and the border dashes span max + 2 = 1. */
...
/* Perl numeric coercion of $opts{'W'}: optional whitespace and sign, then
 * decimal digits; anything else contributes 0.  (Fractional and exponent
 * forms are out of the byte-parity contract; see README.) */
...
int main(int argc, char **argv) {
    ...
    /* Deliberate fix: the reference tests `unless ($ARGV[0])`, so a first
     * remaining argument of "" or "0" is falsy and it silently waits on
     * stdin instead; here any remaining argument selects the argument
     * message. */
    List raw = {0};
    ...
    /* construct_face: sequential ifs, so a later flag in this fixed order
     * overrides an earlier one, and both override -e/-T. */
